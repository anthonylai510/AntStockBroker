...
filePath = './data/MACD_Over_EMA_5+_Days_List.xlsx'
dfA = pd.read_excel(filePath, sheet_name='Sheet1')
dfA = dfA[(dfA['Hit'] == 0)]
#dfA = dfA.tail(5)
dff = dfA.copy()
#print(dff)
dfTicker = dfA.sort_index(ascending=False)
...
def buildWebDriver(symbol):
    ...
    options.add_argument('--user-data-dir=C:\Temp\ChromeProfileMACDOverEMATask{0}'.format(symbol))
...
def GetStockDatafromMoney18(symbol):
    ...
    try:
        ...
        #adjust the DateTime to HK time by adding 15 hours to datetime.now()
        ...
        df['Open'] = np.where(df['Open'].str.contains('-'), df['Close'], df['Open'])
        print(df)

        #open the file for append
        with open('./data/{0}.HK_{1}-{2}-{3}.csv'.format(symbol, EndDate.year, EndDate.month, EndDate.day), 'a') as f:
...
while(True):
    ...
    try:
        # with Parallel(n_jobs=5, prefer="threads") as parallel:
            # retMsg = parallel(delayed(GetStockDatafromMoney18)(ticker) for ticker in dfTicker['Ticker'])
            # print(retMsg)

        # We can use a with statement to ensure threads are cleaned up promptly
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            dictFuture = {executor.submit(GetStockDatafromMoney18, ticker): ticker for ticker in dfTicker['Ticker']}
            #print(dictFuture)

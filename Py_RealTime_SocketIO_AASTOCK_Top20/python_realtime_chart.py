...
##################################
#matplotlib version must be 2.0.2#
...
def animate(i, symbol):
    ...
    df = dd.read_csv('./data/{0}_*.csv'.format(symbol), header=None, names=['DateTime', 'Ticker', 'Close', 'Chg', 'Chg %', 'Volume', 'Turnover', 'P/E', 'Market Cap'])
    df['Volume'] = df['Volume'].apply(value_to_float)
    df['Open'] = df['Close'].head(1).values[0]
